...
export const signUp = (data) => {
  return async (dispatch) => {
    try {
      ...
      const res = await axios.post("http://localhost:5000/usersAuth/signup", user);
      console.log(res);
      dispatch({
        type: AUTH_SIGN_UP,
        payload: res.data,
...
export const signIn = (data) => {
  return async (dispatch) => {
    try {
      const res = await axios.post("http://localhost:5000/userAuth/login", data);
      console.log(res);
      dispatch({
        type: AUTH_SIGN_IN,
        payload: res.data,
...
export const storeFlight = (flightId) => {
  ...
  return async (dispatch) => {
    try {
      const res = await axios.get("http://localhost:5000/transport/" + flightId);
  ...
  return async (dispatch) => {
    try {
      const res = await axios.post(
        "http://localhost:5000/transport/search",
        data
      );
      console.log(res.data);
      if (res.data.length > 0) {
        ...
        dispatch({
          type: FLIGHT_ERROR,
          payload: "Could not get any trips",
        });
      }
      // return res.data;
    } catch (error) {
  ...
  return async (dispatch) => {
    try {
      const res = await axios.get(
        "http://localhost:5000/bookings/userDetails/" + userDetailId
      );
      console.log(res.data);
      dispatch({
        type: GET_BOOKINGS,
